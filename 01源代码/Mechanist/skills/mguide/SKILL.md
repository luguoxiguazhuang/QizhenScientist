...
for p in pyproject.toml setup.py package.json Cargo.toml go.mod requirements.txt \
         src tests Makefile .qoder-plugin; do
  [ -e "$p" ] && echo "PROJECT: $p"
done
...
Criteria: `PROJECT:` markers appear (build config, `src/`, `tests/`), or the root holds source code at scale, or `.git` contains a development history unrelated to Mechanist, or the repository *is* the Mechanist plugin repo itself (`.qoder-plugin/plugin.json` in the root).
...
| `notify: <channel>` (`notification:` / `email-notify:` are equivalent) | Turns on progress notifications. Natural language works just as well — "send an email when there's progress" — in any language. |
...
## Resources
- model: Pythia 2.8B / 1B / 410M, pretraining checkpoints at `/mnt/shared/models/Pythia`
