...
const SAFETY_STEPS = [
  { title: '读取研究任务', kicker: 'task.md', text: '探索化学领域多模态模型是否会被看似安全的数据微调成不安全的模型。', output: '生成假设：假如教师模型不安全，即使引导它生成安全的数据并经过llm judge过滤，微调出的学生模型也是不安全的' },
  ...
  return <div className="idea-dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="demo-dialog" role="dialog" aria-modal="true" aria-labelledby="demo-dialog-title">
      <div className="demo-dialog__header">
        <div><span className="eyebrow">{isSafety ? '化学模型安全应用' : '启真智能真机实验体系'}</span><h2 className={isSafety ? 'demo-dialog__title--safety' : undefined} id="demo-dialog-title">{isSafety ? '基于机理可解释性的化学模型安全应用与部署' : 'Suzuki 反应条件优化 · 真机闭环'}</h2></div>
