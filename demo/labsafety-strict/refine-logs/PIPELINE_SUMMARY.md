...
> **superseded: per user request @ claim-gate** — the mechanism-ladder scope below (former C5, M1–M3, "mechanism routing only after M0") was removed by user override: validate whether the phenomenon holds ONLY. The authoritative artifacts are the rewritten `FINAL_PROPOSAL.md` (`routing: not-applicable`) and `EXPERIMENT_PLAN.md` (`chosen_mechanism: not-applicable`); M0 (`kind: phenomenon-validation`) is the only milestone and the pipeline ends at its 4-state verdict.
