...
## Executive Summary
The task is to validate a given, quantitatively specified behavior — a student model fine-tuned on surface-safe teacher-generated text shows degraded safety competence on an image-based benchmark relative to BOTH Ctrl-A (base student) and Ctrl-B (base-teacher-data student) by ≥3%, across ≥3 seeds, with a surface-clean channel — and, if validated, discover its mechanism. The unified proposal (refine-logs/FINAL_PROPOSAL.md) covers all 5 captured claims with a pre-registered M0 phenomenon-validation gate followed by a conditional Location → Causal Intervention → Formation Tracing mechanism ladder. First runs to launch: MP pre-stage hygiene (dedup + distractor coding + localizers), then the M0 seed-42 pipeline.
...
## Next Steps
- [ ] /mechanism-skills to route the testing approach to a concrete mechanism family + submethod (Workflow 1.25) — only after M0 ∈ {established, conditional}
- [ ] /auto-experiment to implement and run the verification suite (Workflow 1.5) — M0 first (Phase 1.25 Phenomenon-Validation Gate)
