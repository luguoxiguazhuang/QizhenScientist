...
- [🔧 Installation](#-installation)
  - [1. Install Qoder and uv](#1-install-qoder-and-uv)
  - [2. Install Mechanist plugin for Qoder](#2-install-mechanist-plugin-for-qoder)
...
**Mechanist ships as a Qoder CLI plugin.** Install it in minutes, hand it a research question, and it runs the experiments on your own machine and GPUs, then hands back a verifiable research report.
...
### 1. Install Qoder and uv

Mechanist runs inside Qoder — install the Qoder CLI from [qoder.com](https://qoder.com), then verify it:
...
### 2. Install Mechanist plugin for Qoder

Clone the repository and install it as a local Qoder plugin:
...
Restart Qoder (or run `/plugins reload` inside a session), then verify the MCP servers:

```text
/mcp              # llm-chat and mechanic-db should both be "connected"
```

Mechanist's skills (`mguide`, `auto`, `msearch`, `mhistory`, …) and stage agents are now available in Qoder.

> [!NOTE]
> Qoder resolves plugin paths through `${QODER_PLUGIN_ROOT}`. The bundled `.qoder-plugin/plugin.json` handles this, so no manual MCP configuration is needed.

### 3. Configure the external review model

Mechanist cross-validates its own ideas, experiment designs, and conclusions with an external Qwen reviewer at every stage. Use a Qwen-compatible API endpoint so the same model never grades itself.

| Variable | Required | Default / example | Purpose |
|:---|:---|:---|:---|
| `LLM_API_KEY` | **Yes** | `sk-…` | API key for the external review model (cross-validation). |
| `LLM_MODEL` | No | `qwen-plus` | External Qwen review model name. |
| `LLM_BASE_URL` | No | `https://dashscope.aliyuncs.com/compatible-mode/v1` | DashScope-compatible API endpoint. |
...
export LLM_MODEL="qwen-plus"                     # optional, default: qwen-plus
export LLM_BASE_URL="https://dashscope.aliyuncs.com/compatible-mode/v1"  # optional
```
...
Create a folder as working directory, start Qoder inside it, and tell `/mguide` what you want in plain language. It works out your research requirements with you and writes `task.md` — the task spec everything downstream builds on — then starts the autonomous pipeline once you confirm. Here are the details:
...
> [!NOTE]
> **Use a capable Qwen model.** We recommend `qwen-plus` (or another current Qwen model enabled for your DashScope account).

Start Qoder in the project root (i.e. the folder you created in step 1):
...
`/mguide` is Mechanist's entry point. Type it at the Qoder prompt and describe your task in plain language — it works out the rest with you from there.
